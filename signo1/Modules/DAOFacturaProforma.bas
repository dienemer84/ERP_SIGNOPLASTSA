Attribute VB_Name = "DAOFacturaProforma"
Option Explicit
...
Public Function FindAll(Optional ByVal filter As String = "1 = 1", Optional includeDetalles As Boolean = False, Optional includeEntregasWithDetalles As Boolean = False, Optional Orden As String = vbNullString) As Collection
    ...
    On Error GoTo err1
    Dim q As String
    q = "SELECT *, ADDDATE(AdminFacturasProformas.FechaEmision, AdminFacturasProformas.FormaPago) AS FechaVencimiento " _
...
If includeDetalles Then
        ...
        q = q & ",CAST((SELECT   GROUP_CONCAT(DISTINCT r.numero SEPARATOR ',') AS lista_remitos FROM AdminFacturasDetalleAplicacionRemitos a " _
            & "INNER JOIN entregas e ON e.id=a.idRemitoDetalle INNER JOIN remitos r ON e.Remito = r.id  WHERE a.idFacturaDetalle= AdminFacturasProformaDetalleNueva.id) AS CHAR) as lista_remitos_aplicados "


        q = q & ",CAST((SELECT   COUNT(DISTINCT r.numero) AS cantidad_remitos FROM AdminFacturasDetalleAplicacionRemitos a " _
            & "INNER JOIN entregas e ON e.id=a.idRemitoDetalle INNER JOIN remitos r ON e.Remito = r.id  WHERE a.idFacturaDetalle= AdminFacturasProformaDetalleNueva.id) AS CHAR) as cantidad_remitos_aplicados "
    End If

    q = q & " ,  CONVERT((SELECT IFNULL(GROUP_CONCAT(idRecibo),'-') FROM AdminRecibosDetalleFacturas INNER JOIN AdminRecibos ON AdminRecibosDetalleFacturas.idRecibo = AdminRecibos.id WHERE AdminRecibosDetalleFacturas.idFactura = AdminFacturasProformas.id),NCHAR) AS nro_recibo "

    q = q & " From AdminFacturasProformas" _
        & " LEFT JOIN AdminConfigFacturasTiposDiscriminado acftd      ON (       acftd.id = AdminFacturasProformas.id_tipo_discriminado    ) " _
        ...
        & " LEFT JOIN clientes ON (AdminFacturasProformas.idCliente = clientes.id)" _
        ...
        & " LEFT JOIN AdminConfigMonedas ON (AdminFacturasProformas.idMoneda = AdminConfigMonedas.id)" _
        & " LEFT JOIN usuarios ON AdminFacturasProformas.idUsuarioEmision=usuarios.id " _
        & " LEFT JOIN usuarios as usuarios2 ON AdminFacturasProformas.idUsuarioAprobacion=usuarios2.id " _
        & " LEFT JOIN ( " _
        & " SELECT idFactura, idNC, id FROM AdminFacturas_NC " _
        & " Union All " _
        & " SELECT idNC, idFactura, id FROM AdminFacturas_NC " _
        & " ) AS fnc_combined ON fnc_combined.idFactura = AdminFacturasProformas.id "

    If includeDetalles Then
        q = q & " LEFT JOIN  AdminFacturasProformaDetalleNueva ON AdminFacturasProformaDetalleNueva.idFactura = AdminFacturasProformas.id "
        If includeEntregasWithDetalles Then
            q = q & " LEFT JOIN  entregas ON entregas.id = AdminFacturasProformaDetalleNueva.idEntrega "
        End If
    End If
    ...
    While Not rs.EOF
        Set F = Map(rs, idx, "AdminFacturasProformas", "clientes", "AdminConfigMonedas", "iva", "acftd", "ivaFac", "acft", "pv", "fnc")
...
'        F.RecibosAplicadosId = rs!nro_recibo
        ...
        If includeDetalles Then
            Set deta = DAOFacturaProformaDetalles.Map(rs, idx, "AdminFacturasProformaDetalleNueva")
...
Public Function FindById(id As Long, Optional includeDetalles As Boolean = False, Optional includeEntregasWithDetalles As Boolean = False) As clsFacturaProforma
    Dim col As Collection: Set col = FindAll("AdminFacturasProformas.id = " & id, includeDetalles, includeEntregasWithDetalles)
    If col.count = 0 Then
...
Public Function Save(FacturaProforma As clsFacturaProforma, Optional Cascade As Boolean = False) As Boolean
    On Error GoTo err1

    conectar.BeginTransaction
    Save = True
    If Not Guardar(FacturaProforma, Cascade) Then
        GoTo err1
    End If
...
Public Function Guardar(F As clsFacturaProforma, Optional Cascade As Boolean = False) As Boolean
    Dim q As String
    Dim esNueva As Boolean

    If F.id > 0 Then
        q = "UPDATE sp.AdminFacturasProformas  SET " _
            & " NroFactura = 'NroFactura' , idCliente = 'idCliente'," _
    ...
    Else
        ...
        q = "INSERT INTO sp.AdminFacturasProformas " _
            & " (NroFactura, " _
            & " idCliente, " _
            & " idMoneda, " _
            ...
            & " propuesta, fecha_serv_desde, fecha_serv_hasta, " _
...
'    q = Replace$(q, "'tipo_borrar'", conectar.Escape(F.TipoDocumento))
    ...
    If conectar.execute(q) Then
        ...
        If DAOFacturaProforma.FindAll("AdminFacturasProformas.NroFactura = " & F.numero & " And AdminFacturasProformas.Id <> " & F.id).count > 0 Then
    ...
    Dim Ot As OrdenTrabajo
      ...
      If Cascade Then
        Dim det As clsFacturaProformaDetalle
        DAOFacturaProformaDetalles.Delete "idFactura=" & F.id

        For Each det In F.Detalles
            det.id = 0
            det.idFactura = F.id
            If Not DAOFacturaProformaDetalles.Guardar(det) Then
...
Public Function aprobarV2(Factura As clsFacturaProforma, aprobarLocal As Boolean, enviarAfip As Boolean) As Boolean
    On Error GoTo err5

    Set Factura = DAOFacturaProforma.FindById(Factura.id)
    ...
    If aprobarLocal Then
        ...
        Factura.Detalles = DAOFacturaProformaDetalles.FindByFactura(Factura.id)    'DAOFactura.FindById(Factura.id, True)
        Dim d As clsFacturaProformaDetalle
        ...
        Dim T As clsFacturaProforma
        Set T = Factura
        
        If Not DAOFacturaProforma.Guardar(Factura) Then GoTo err5
...
Public Function Imprimir(idFactura As Long) As Boolean
    ...
    Dim objFac As clsFacturaProforma
    Set objFac = DAOFacturaProforma.FindById(idFactura, True, True)
    ...
    conectar.execute "update AdminFacturasProformas set impresa=impresa+1 where id=" & idFactura
