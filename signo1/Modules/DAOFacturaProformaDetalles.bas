Attribute VB_Name = "DAOFacturaProformaDetalles"
     Option Explicit

Public Function Guardar(detalle As clsFacturaProformaDetalle) As Boolean
    On Error GoTo err1
    Guardar = True
    Dim q As String
    If detalle.id > 0 Then
        q = "Update sp.AdminFacturasProformaDetalleNueva  SET " _
    ...
    Else
        q = " INSERT INTO sp.AdminFacturasProformaDetalleNueva " _
...
Public Function Save(deta As clsFacturaProformaDetalle) As Boolean
    Save = True
...
Public Function FindAll(Optional filtro As String = vbNullString, Optional withFacturaNoLazy As Boolean = False) As Collection
    ...
    Dim F As clsFacturaProformaDetalle
...
Public Function Map(rs As Recordset, indice As Dictionary, tablaFacturaDetalle As String, Optional tablaEntrega As String = vbNullString) As clsFacturaProformaDetalle
    Dim id As Long
    Dim F As clsFacturaProformaDetalle
    id = GetValue(rs, indice, tablaFacturaDetalle, "id")

    If id > 0 Then
        Set F = New clsFacturaProformaDetalle
        F.id = id
...
Public Function Delete(filter As String) As Boolean
    Delete = conectar.execute("DELETE FROM AdminFacturasProformaDetalleNueva WHERE " & filter)
End Function
