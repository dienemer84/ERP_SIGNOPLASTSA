...
Public Function Guardar(fc As clsFacturaProveedor) As Boolean
    ...
    If fc.FEcha <= fecha_liqui_max Then
        MsgBox "La fecha del comprobante es inv谩lida ya que corresponde a un periodo ya liquidado", vbCritical, "Error"
        Exit Function
    End If
    ...
    Else
        ...
        If fca.UltimaActualizacion > Now Then Err.Raise 104, "fc", "La factura fu茅 guardada en otra sesi贸n, por favor actualice y vuelva a realizar la operaci贸n"
...
err1:
    Guardar = False
    If Err.Number = 100 Then MsgBox "Se produjo algun error, no se  guadar谩n los cambios!"
...
Public Function aprobar(fc As clsFacturaProveedor) As Boolean
    ...
    If fc.estado = EstadoFacturaProveedor.EnProceso Then
        ...
        End If
    Else
        '  MsgBox "No puede cambiar el estado de la factura, ya fue aprobada!", vbInformation, "Informaci贸n"
        Err.Raise 4431, "Aprobar factura", "Error: La factura fu茅 aprobada en otra sesi贸n "
...
Public Function Delete(facid As Long) As Boolean
    ...
    If facsOrphan.count > 0 Then
        MsgBox "El comprobante " & facProv.NumeroFormateado & " fue eliminado as铆 como tambi茅n la Orden de Pago N潞 " & op.Id & " a la que pertenecia." & vbNewLine & "Los siguientes comprobantes formaban parte de esa orden de pago y ahora se encuentran en estado [Pendiente] para ser incluidos en alguna otra orden de pago:" & vbNewLine & funciones.JoinCollectionValues(facsOrphan, vbNewLine, "NumeroFormateado"), vbInformation + vbOKOnly
    Else
        If IsSomething(op) Then
            MsgBox "El comprobante " & facProv.NumeroFormateado & " fue eliminado as铆 como tambi茅n la Orden de Pago N潞 " & op.Id & " a la que pertenecia.", vbInformation + vbOKOnly
        End If
    End If
...
Public Function CrearTablaTempComprobantes(facturas) As Boolean
    ...
    For Each fac In facturas
        ...
        puntoDeVenta = CStr(CLng(partes(0))) ' Convertir a n鷐ero sin procesar
        numeroDesde = CStr(CLng(partes(1))) ' Convertir a n鷐ero sin procesar
...
Public Function ExportarColeccionTotalizadores(col As Collection, Optional ProgressBar As Object, Optional FechaFIn As String) As Boolean
    ...
    xlWorksheet.Cells(offset, 6).value = "N鷐ero"
