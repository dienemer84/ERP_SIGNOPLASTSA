...
Private m_oSaxWriter As MSXML2.MXXMLWriter
...
Private m_oSaxReader As MSXML2.SAXXMLReader
...
Attribute SerializeObject.VB_Description = "recursive loop through objects to serialize them to xml"
    '--------------------------------------------------------------------------------
    ...
    Dim oAttribute As MSXML2.SAXAttributes
    Set oAttribute = New MSXML2.SAXAttributes
    ...
    For Each oMember In oInterface.Members
        ...
        If oMember.InvokeKind = VbGet And oMember.Parameters.count = 0 _
            ...
            ElseIf GetObjectProperty(oSourceObject, sMemberName, oChild) Then
                If Not oChild Is Nothing Then
                    SerializeObject oChild 'recursive call to serialize
                End If
            End If
...
Attribute Serialize.VB_HelpID = 2007
    ...
    Set m_oSaxWriter = New MSXML2.MXXMLWriter
