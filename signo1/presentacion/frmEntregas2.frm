...
Public Sub SetOrdenTrabajo(Ot As OrdenTrabajo)
    Set m_ot = Ot
    Me.caption = "Entregas de la OT N� " & m_ot.IdFormateado
    CargaDetalles
End Sub
...
Private Sub CargaDetalles()
    ...
    Me.lblPorcentajeFabricacion.caption = "% Fabricaci�n: " & m_ot.PorcentajeFabricacion
...
Private Sub btnCerrar_Click()
    If m_ot.TodoEntregado Then
        ...
        If DAOOrdenTrabajo.Cerrar(m_ot) Then
            ...
            MsgBox "La OT " & m_ot.Id & " se cerr� correctamente.", vbInformation
...
Private Sub btnExportarExcel_Click()
    ...
    xlWorksheet.Cells(1, 2).value = "OT N� " & m_ot.IdFormateado & " al dia " & Date
...
Private Sub btnTomarDeStock_Click()
    ...
    'bug #3
    Dim res As String
    res = InputBox("Ingrese la cantidad a tomar de stock (M�ximo " & detalle.Pieza.CantidadStock & ")", "Reserva de Stock", "0")
...
Private Sub cmdPreconteo_Click()
    If Not Permisos.PlanRemitosControl Then
        MsgBox "No tiene permisos para realizar esta acci�n!", vbCritical, "Error"
        Exit Sub
    End If
...
Private Sub ImprimirEntregas()
    ...
    Printer.Print "DETALLE DE ENTREGAS O/T Nro " & m_ot.IdFormateado & " al d�a " & Date
    ...
    'Ac� se imprimen los encabezados de la Lista
    Printer.Print Tab(1);
    Printer.Print "Item";
    ...
    Printer.Print "Fecha emisi�n " & Date
        ...
        If EVENTO.Elemento.count > 0 And IsSomething(detalle) Then
            ...
            If redeta.Cantidad <= (detalle.CantidadPedida - detalle.CantidadEntregada) Then
                ...
                If redeta.Origen = OrigenRemitoConcepto Then
                    If MsgBox("�Est� seguro de aplicar este remito a este item de la OT?", vbYesNo + vbQuestion) = vbYes Then
                        If claseP.aplicarRemitoAOT(m_ot.Id, redeta.Id, detalle.Id, redeta.Cantidad) Then
                            ...
                            CargaDetalles
                        Else
                            MsgBox "Se produjo alg�n error. No se graban los cambios.", vbCritical
                        End If
                    End If
                Else
...
Private Sub mnuArchivoPieza_Click()
    ...
    F.caption = "OT N� " & m_ot.IdFormateado & " - Item " & detalle.item
    F.Show
End Sub
...
Private Sub mnuAtajo_Click()
    ...
    If MsgBox("�Seguro de crear un remito r�pido para esta OT?", vbYesNo, "Consulta") = vbYes Then
        rtoRapido = True
        On Error GoTo err1
        conectar.BeginTransaction
